...
var CrashManager = AbstractService.extend({
    ...
    show_error: function (error) {
        ...
        var dialog = new dialogClass(this, {
            title: _.str.capitalize(error.type) || _t("User Error"),
        }, error);
...
var RedirectWarningHandler = Widget.extend(ExceptionHandler, {
    ...
    display: function() {
        ...
        new WarningDialog(this, {
            title: _.str.capitalize(error.type) || _t("Cpabooks Warning"),
            buttons: [
...
function session_expired(cm) {
    return {
        display: function () {
            const notif = {
                type: _t("Cpabooks Session Expired"),
                message: _t("Your Cpabooks session expired. The current page is about to be refreshed."),
            };
            const options = {
                buttons: [{
                    text: _t("Ok"),
