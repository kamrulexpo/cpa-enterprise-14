...
class AdyenAccount(models.Model):
    ...
    def create(self, values):
        ...
        # Create account on cpabooks.org, proxy and Adyen
        response = adyen_account_id._adyen_rpc('create_account_holder', adyen_account_id._format_data())

        # Save adyen_uuid and proxy_token, that have been generated by cpabooks.org and the proxy
    ...
    def action_create_redirect(self):
        ...
        The action will redirect the user to accounts.cpabooks.org to link an Odoo user_id to the Adyen
        account. After logging in on cpabooks.org the user will be redirected to his DB with a token in
