                                ...
                                .then(function (ids) {
                                    if (ids.length === 0) { // there are no dirty ids on the server
                                        ...
                                        if (!self.data.warning_date || (self.data.warning_date && moment(self.data.warning_date).diff(new Date(), 'days') !== 0)) {
                                            alert('The code on CpaBooks server is not up to date and an important update has been released. You or your system administrator should consider retrieving it as soon as possible.');
                ...
                if (response.length > 0 && _.contains(response, 'project_timesheet_synchro')) {
                    ...
                    session.rpc('/jsonrpc',  { method : 'server_version' , service : 'db', args : []}).then(function(result) {
                        if (result && result.endsWith('e')) {
                            ...
                            alert("Timesheet Synchronization is available in Odoo Enterprise Edition. You should consider upgrading CpaBooks version if you would like to use it.");
                        } else {
